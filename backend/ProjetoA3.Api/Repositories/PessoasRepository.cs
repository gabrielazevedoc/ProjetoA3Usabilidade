...
public class PessoasRepository : IPessoasRepository
{
    ...
    public async Task<PessoaFisica> CreateAsync(PessoaCreateRequest request)
    {
        ...
        const string sql = @"INSERT INTO PessoasFisicas
    ...
    public async Task<PagedResult<PessoaListItem>> ListAsync(PaginationQuery query)
    {
        ...
        const string sql = @"SELECT Id, Nome, Telefone, Email, Latitude, Longitude, TipoResiduo, QuantidadeKg, Observacoes, CreatedAt
                             ...
                             SELECT COUNT(*) FROM PessoasFisicas;";
    ...
    {
        const string sql = "SELECT * FROM PessoasFisicas WHERE Id = @Id";
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync();
    ...
    {
        const string sql = "SELECT * FROM PessoasFisicas WHERE Email = @Email";
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync();
    ...
    public async Task<PessoaFisica?> UpdateAsync(int id, PessoaUpdateRequest request)
    {
        ...
        var sql = $"UPDATE PessoasFisicas SET {string.Join(", ", fields)} WHERE Id = @Id";
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync();
    ...
    {
        const string sql = "DELETE FROM PessoasFisicas WHERE Id = @Id";
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync();
    ...
    {
        const string sql = "SELECT Id, Nome, Telefone, Email FROM PessoasFisicas WHERE Id = @Id";
        await using var connection = await _connectionFactory.CreateOpenConnectionAsync();
